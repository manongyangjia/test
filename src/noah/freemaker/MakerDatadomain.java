...
public class MakerDatadomain extends TemplateObject {
	private List<DbTablename> columnsList;

	public MakerDatadomain(Map<String, Object> root, List<DbTablename> columnsList) {
		this.root = root;
		this.columnsList = columnsList;
	}

	public void generateFile(String filePath, String fileName, Map<String, Object> root) throws IOException, TemplateException {
		...
		Template temp = cfg.getTemplate("dataDomain.ftlh");
		Writer out = new OutputStreamWriter(new FileOutputStream(fileName), "UTF-8");
		temp.process(root, out);

	}

	public List<TabColumn> getDomailColumn(List<DbTablename> db) {
		...
		if (ret != null && ret.size() > 0) {
			for (TabColumn data : ret) {
				if (data.getColumnName().contains("_")) {
					data.setColumnName(data.getColumnName().replaceAll("_", ""));
				}
			}
		}
		return ret;
	...
	public void gengerateCode() {
		try {
			root.put("allColunm", getDomailColumn(columnsList));
