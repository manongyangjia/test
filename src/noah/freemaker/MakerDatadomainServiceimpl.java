...
import noah.datasource.Table;
import freemarker.template.Configuration;
import freemarker.template.Template;
import freemarker.template.TemplateException;
import freemarker.template.TemplateExceptionHandler;

public class MakerDatadomainServiceimpl  extends TemplateObject{
	private List<DbTablename> columnsList;
	public MakerDatadomainServiceimpl(Map<String, Object> root, List<DbTablename> columnsList) {
		this.root = root;
	...
	public static void generateFile(String filePath, String fileName, Map<String, Object> root) throws IOException, TemplateException {
		...
		Template temp = cfg.getTemplate("dataDomainserviceimpl.ftlh");
	...
	public void gengerateCode() {
		...
		String fileName = (String) root.get("fileName");
		String pathName1 = pathName + packageName.replace(".", "/");
		String fileName1 = pathName + packageName.replace(".", "/") + "/" + fileName + ".java";
		try {
			generateFile(pathName1, fileName1, root);
		} catch (IOException e) {
			e.printStackTrace();
